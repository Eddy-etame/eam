    ...
    <Link href={localizedPath(locale, `work/${project.slug}`)} className="group block">
      <article>
        ...
        >
          ...
          <span className="text-mono-label absolute left-4 top-4 text-white/70">
            {categoryLabels[project.category][locale]}
          </span>
          <span className="text-mono-label absolute bottom-4 right-4 text-white/55">
            {project.year}
          </span>
          <span
            aria-hidden
            className="absolute inset-0 rounded-lg ring-1 ring-inset ring-transparent transition-colors duration-500 group-hover:ring-gold/55"
          />
        </div>
        <div className="mt-5 flex items-start justify-between gap-4">
          ...
          <span
            aria-hidden
            className="mt-1.5 shrink-0 text-gold opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          >
            →
          </span>
        </div>
